		...
		void UpdateAI(uint32 diff) override
		{
			...
			/*if (me->HasUnitState(UNIT_STATE_CASTING))
				...
				case EVENT_CHALLENGE_DONE:
					...
					instance->DoUpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 68575, 0, me);
...
public:
	...
	struct boss_paletressAI : public argent_challenge_baseAI
	{
		...
		void DamageTaken(Unit* /*done_by*/, uint32 &damage) override
		{
			...
			if (!_memorySummoned && !HealthAbovePct(35))
			{
		...
		void UpdateAI(uint32 diff) override
		{
			...
			/*if (me->HasUnitState(UNIT_STATE_CASTING))
				...
				case EVENT_ENTER_AGGRESSIVE:
					...
					_events.ScheduleEvent(EVENT_OLD_WOUNDS, 11s, 13s);
					_events.ScheduleEvent(EVENT_SHADOWS_PAST, 5s);
					_events.ScheduleEvent(EVENT_WAKING_NIGHTMARE, 7s, 10s);
					break;
...
class achievement_eadric_killer : public AchievementCriteriaScript
{
public:
	achievement_eadric_killer(char const* name, uint32 criteriaId) : AchievementCriteriaScript(name),
	...
	bool OnCheck(Player* /*player*/, Unit* target) override
	{
		if (target)
		{
			...
			case 12324:
			case 12316:
				return target->GetEntry() == NPC_EADRIC;
			default:
				break;
			}
		}
...
void AddSC_boss_argent_challenge()
{
	...
	new achievement_eadric_killer("achievement_eadric_killer", 12308);
	new achievement_eadric_killer("achievement_eadric_killer", 11560);
	new achievement_eadric_killer("achievement_eadric_killer", 12324);
	new achievement_eadric_killer("achievement_eadric_killer", 12316);
}
