...
enum Spells
{
	...
	SPELL_TRAMPLE_DUMMY_AURA = 67867, // triggers trample aura every 1 second, used by all vehicles
	SPELL_TRAMPLE_AURA = 67867, // actual trample stun
...
struct boss_grand_championAI : BossAI
{
	...
	void Reset() override
	{
		...
		me->ApplySpellImmune(SPELL_TRAMPLE_AURA, IMMUNITY_ID, SPELL_TRAMPLE_AURA, true);
	}
	...
	void SpellHit(WorldObject* caster, SpellInfo const* spellInfo) override
	{
		if (spellInfo->Id == SPELL_TRAMPLE_AURA && LookingForMount && uiPhase == 0 && !me->IsImmunedToSpell(spellInfo, caster))
		{
			...
			events.ScheduleEvent(EVENT_TRAMPLE, 3s);
		}
	}
			...
			if (Player* plr = itr->GetSource())
				if (me->GetExactDist(plr) <= 5.0f)
					if (Vehicle* v = plr->GetVehicle())
						if (Unit* c = v->GetBase())
							if (c->GetTypeId() == TYPEID_UNIT && c->ToCreature()->GetEntry() ==  TEAM_HORDE ? 36558 : 35644)
							{
								me->CastSpell(me, SPELL_TRAMPLE_AURA, false);
								break;
							}
	}*/
	...
	void UpdateAI(uint32 diff) override
	{
		events.Update(diff);
			...
			case EVENT_TRAMPLE:
				...
				break;
			case EVENT_EVADE_MODE:              				// Entering to evade mode after jousting event is done
				bHome = true;
				EnterEvadeMode();
				break;
...
public:
	...
	struct generic_vehicleAI_toc5AI : public EscortAI
	{
		...
		{
			castedPennant = false;
			events.RescheduleEvent(EVENT_BUFF_FLAG, 2s);
			events.RescheduleEvent(EVENT_BUFF_SHIELD, 5s);
			events.RescheduleEvent(EVENT_MOUNT_CHARGE, 5s);
			events.RescheduleEvent(EVENT_SHIELD_BREAKER, 8s);
		}
		...
		void UpdateAI(uint32 uiDiff) override
		{
			...
			events.Update(uiDiff);
				...
				case EVENT_BUFF_SHIELD:
					DoCastShield();
					events.RescheduleEvent(EVENT_BUFF_SHIELD, 7s);
					break;
				...
				case EVENT_MOUNT_CHARGE:
				{
					...
					events.RescheduleEvent(EVENT_MOUNT_CHARGE, 7s);
					break;
				}
				case EVENT_SHIELD_BREAKER:
					...
					if (IsGrandChampionVehicle() && me->GetVehicleKit())
					{
						...
						events.RescheduleEvent(EVENT_SHIELD_BREAKER, 4s);
					}
					break;
				default:
					break;
...
public:
	...
	struct boss_warrior_toc5AI : public boss_grand_championAI
	{
		...
		void JustEngagedWith(Unit* who) override
		{
			events.RescheduleEvent(EVENT_BLADESTORM, 15s);
			events.RescheduleEvent(EVENT_INTERCEPT, 7s);
			events.RescheduleEvent(EVENT_MORTAL_STRIKE, 10s);
			events.RescheduleEvent(EVENT_ROLLING_THROW, 20s);
		...
		void UpdateAI(uint32 uiDiff) override
		{
			...
			/*if (me->HasUnitState(UNIT_STATE_CASTING))
				...
				case EVENT_BLADESTORM:
					...
					events.RescheduleEvent(EVENT_BLADESTORM, 20s);
					break;
				case EVENT_MORTAL_STRIKE:
					if (!me->HasAura(SPELL_BLADESTORM))
						DoCastVictim(SPELL_MORTAL_STRIKE);
					events.RescheduleEvent(EVENT_MORTAL_STRIKE, 12s);
					break;
				case EVENT_ROLLING_THROW:
					...
					events.RescheduleEvent(EVENT_ROLLING_THROW, 30s);
					break;
				default:
					break;
				}
...
public:
	...
	struct boss_mage_toc5AI : public boss_grand_championAI
	{
		...
		void JustEngagedWith(Unit* who) override
		{
			events.RescheduleEvent(EVENT_FIREBALL, 2s);
			events.RescheduleEvent(EVENT_POLYMORPH, 8s);
			events.RescheduleEvent(EVENT_BLASTWAVE, 12s);
			events.RescheduleEvent(EVENT_HASTE, 22s);
			boss_grand_championAI::JustEngagedWith(who);
		}
		...
		void UpdateAI(uint32 uiDiff) override
		{
			...
			if (CanUseNormalAI())
			{
				...
				case EVENT_BLASTWAVE:
					...
					events.RescheduleEvent(EVENT_BLASTWAVE, 13s);
					break;
				case EVENT_HASTE:
					me->InterruptNonMeleeSpells(true);
...
public:
	...
	struct boss_shaman_toc5AI : public boss_grand_championAI
	{
		...
		void JustEngagedWith(Unit* who) override
		{
			events.RescheduleEvent(EVENT_DEF_CHECK, 5s);
			events.RescheduleEvent(EVENT_CHAIN_LIGHTNING, 16s);
			events.RescheduleEvent(EVENT_HEALING_WAVE, 12s);
			events.RescheduleEvent(EVENT_EARTH_SHIELD, 5s);
			events.RescheduleEvent(EVENT_HEX_MENDING, 7s);
			events.RescheduleEvent(EVENT_EARTH_SHIELD, 10s);
			events.RescheduleEvent(EVENT_HEALING_WAVE, 15s);
			boss_grand_championAI::JustEngagedWith(who);
		}

		void JustEngagedWithMode(bool interrupt)
		{
			if (interrupt)
				me->InterruptNonMeleeSpells(true);
			isDefensive = false;
			events.RescheduleEvent(EVENT_CHAIN_LIGHTNING, 1s);
		}
		...
		void EnterDefensiveMode()
		{
			me->InterruptNonMeleeSpells(true);
			isDefensive = true;
			events.RescheduleEvent(EVENT_EARTH_SHIELD, 1s);
			events.RescheduleEvent(EVENT_HEALING_WAVE, 2s);
		}
			...
			if (CanUseNormalAI())
			{
				...
				case EVENT_DEF_CHECK:
					...
					else
					{
						Unit* pFriend = FindChampionWithLowestHp(40.0f);
						if (pFriend && pFriend->GetHealthPct() > 25)
							JustEngagedWithMode(false);
						else
							JustEngagedWithMode(true);
					}
					events.RescheduleEvent(EVENT_DEF_CHECK, 5s);
					break;
				...
				case EVENT_EARTH_SHIELD:
					...
					events.RescheduleEvent(EVENT_EARTH_SHIELD, 35s);
					break;
				case EVENT_HEX_MENDING:
					DoCastVictim(SPELL_HEX_OF_MENDING);
					events.RescheduleEvent(EVENT_HEX_MENDING, 25s);
					break;
				default:
					break;
				}
...
public:
	...
	struct boss_hunter_toc5AI : public boss_grand_championAI
	{
		...
		void JustEngagedWith(Unit* who) override
		{
			events.RescheduleEvent(EVENT_MULTI_SHOT, 7s);
			events.RescheduleEvent(EVENT_LIGHTNING_ARROWS, 15s);
			events.RescheduleEvent(EVENT_DISENGAGE, 30s);
			boss_grand_championAI::JustEngagedWith(who);
		}
		...
		void UpdateAI(uint32 uiDiff) override
		{
			...
			if (CanUseNormalAI())
			{
				...
				case EVENT_DISENGAGE:
					...
					events.RescheduleEvent(EVENT_DISENGAGE, 35s);
					break;
				case EVENT_LIGHTNING_ARROWS:
					...
					events.RescheduleEvent(EVENT_LIGHTNING_ARROWS, 30s);
					break;
				case EVENT_MULTI_SHOT:
					...
					events.RescheduleEvent(EVENT_MULTI_SHOT, 8s);
					break;
				default:
					break;
				}
...
public:
	...
	struct boss_rogue_toc5AI : public boss_grand_championAI
	{
		...
		void JustEngagedWith(Unit* who) override
		{
			events.RescheduleEvent(EVENT_DEADLY_POISON, 1s);
			events.RescheduleEvent(EVENT_EVISCERATE, 8s);
			events.RescheduleEvent(EVENT_FAN_OF_KNIVES, 14s);
			events.RescheduleEvent(EVENT_POISON_BOTTLE, 19s);
			boss_grand_championAI::JustEngagedWith(who);
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			/*if (me->HasUnitState(UNIT_STATE_CASTING))
				...
				case EVENT_DEADLY_POISON:
					...
					events.RescheduleEvent(EVENT_DEADLY_POISON, 31s);
